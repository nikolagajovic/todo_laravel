            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('todos.store') }}" class="mb-6 space-y-4">
                        @csrf
                        <div>
                            <input type="text" name="task" placeholder="Unesite novi zadatak..." class="w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <!-- Rok (due date) -->
                            <input type="datetime-local" name="due_date" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            <!-- Tajmer (u minutima) -->
                            <input type="number" name="duration_minutes" placeholder="Tajmer (min)" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" min="1">
                        </div>
                    ...
                    <div class="space-y-4">
                        @forelse ($todos as $todo)
                            <div class="flex items-center justify-between p-4 rounded-lg
                                ...
                                <div>
                                    <span class="{{ $todo->status !== 'pending' ? 'line-through text-gray-500' : 'text-gray-800' }}">
                                        {{ $todo->task }}
                                    </span>
                                    @if($todo->category)
                                        <span class="ml-2 text-xs text-white bg-blue-500 px-2 py-1 rounded-full align-middle">{{ $todo->category->name }}</span>
                                    @endif
                                ...
                                <div class="flex items-center space-x-2 flex-shrink-0">
                                    ...
                                    <form method="POST" action="{{ route('todos.update', $todo) }}" class="mark-done-form">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="text-sm text-green-600 hover:text-green-900 font-semibold">Urađeno</button>
                                    </form>
                                    @endif
                                    ...
                                    <form method="POST" action="{{ route('todos.destroy', $todo) }}" onsubmit="return confirm('Jeste li sigurni?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-sm text-red-600 hover:text-red-900 font-semibold">Obriši</button>
                                    </form>
                                </div>
                            </div>
                        @empty
            ...
            const failTask = (taskId, timerElement) => {
                ...
                fetch(url, {
                    ...
                    if(response.ok) {
                        ...
                        timerElement.parentElement.innerHTML = '<span class="font-bold text-red-700">Neurađen (vreme isteklo)</span>';
                    }
                });
            };
            ...
            timers.forEach(timerEl => {
                ...
                const intervalId = setInterval(() => {
                    ...
                    if (remainingTime <= 0) {
                        ...
                        timerEl.textContent = `Preostalo vreme: ${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
                    }
                }, 1000);
            });
        });
